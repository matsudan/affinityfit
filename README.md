...
SPR、マイクロアレイ蛍光、プレートリーダー、レポーターアッセイ、酵素反応速度など、
測定装置に依らず同じ数式（飽和曲線）を使う実験を対象にしています。
...
| `asymptotic` | 共分散行列の曲率から対称な区間を出す。高速だが、識別不能なデータでは標準誤差が発散し、濃度の下限が負になるなど破綻する |
...
実装済み: Hill 係数つきモデル、グローバルフィット（パラメータの共有・固定）、AICc、
プロファイル尤度とブートストラップによる信頼区間、不確かさに応じた丸め。

- 時間経過モデル `A(1 - exp(-k·t))`。振幅が決定不能なときの警告つき
- 複数データセットの結果表を CSV / JSON で出力するヘルパ
- 競合阻害モデル、tight-binding（二次式）モデル
- 山型のプロファイル（金属濃度の最適値、pH 最適）
- LOD / LOQ の算出（IUPAC / CLSI 準拠）
- SPR センサーグラムや各種プレートリーダー形式のインポータ
